...
// X returns the x member of the Vector2
func (v *Vector2) X() *big.Float {
	return v.x
}

// Y returns the y member of the Vector2
func (v *Vector2) Y() *big.Float {
	return v.y
}

// Marshal converts the Vector2 into a slice of bytes
...
// Clone creates a copy of this Vector2
func (v *Vector2) Clone() d2interface.Vector2 {
	result := &Vector2{}
	result.Copy(v)
...
// Copy copies the src x/y members to this Vector2 x/y members
...
func (v *Vector2) SetFromEntity(entity d2interface.WorldEntity) d2interface.Vector2 {
	return v.Copy(entity.Position())
}

// Set the x,y members of the Vector2
func (v *Vector2) Set(x, y *big.Float) d2interface.Vector2 {
	v.x = x
	v.y = y
...
func (v *Vector2) SetToPolar(azimuth, radius *big.Float) d2interface.Vector2 {
...
func (v *Vector2) Equals(src d2interface.Vector2) bool {
	return v.x.Cmp(src.X()) == 0 && v.y.Cmp(src.Y()) == 0
}

// FuzzyEquals checks if the Vector2 is approximately equal
// to the given Vector2. epsilon is what we consider `smol enough`
func (v *Vector2) FuzzyEquals(src d2interface.Vector2) bool {
...
func (v *Vector2) Abs() d2interface.Vector2 {
	clone := v.Clone()
...
func (v *Vector2) Angle() *big.Float {
	...
	floatAngle := math.Atan2(floatX, floatY)
...
// SetAngle sets the angle of this Vector2
func (v *Vector2) SetAngle(angle *big.Float) d2interface.Vector2 {
	return v.SetToPolar(angle, v.Length())
}

// Add to this Vector2 the components of the given Vector2
func (v *Vector2) Add(src d2interface.Vector2) d2interface.Vector2 {
	v.x.Add(v.x, src.X())
	v.y.Add(v.y, src.Y())
...
// Subtract from this Vector2 the components of the given Vector2
func (v *Vector2) Subtract(src d2interface.Vector2) d2interface.Vector2 {
	v.x.Sub(v.x, src.X())
	v.y.Sub(v.y, src.Y())
...
// Multiply this Vector2 with the components of the given Vector2
func (v *Vector2) Multiply(src d2interface.Vector2) d2interface.Vector2 {
	v.x.Mul(v.x, src.X())
	v.y.Mul(v.y, src.Y())
...
func (v *Vector2) Scale(s *big.Float) d2interface.Vector2 {
	v.x.Sub(v.x, s)
	v.y.Sub(v.y, s)
...
// Divide this Vector2 by the given Vector2
func (v *Vector2) Divide(src d2interface.Vector2) d2interface.Vector2 {
	v.x.Quo(v.x, src.X())
	v.y.Sub(v.y, src.Y())
...
// Negate thex and y components of this Vector2
func (v *Vector2) Negate() d2interface.Vector2 {
...
// DistanceSq calculate the distance suared between this Vector2 and the given
// Vector2
...
// SetLength sets the length of this Vector2
func (v *Vector2) SetLength(length *big.Float) d2interface.Vector2 {
...
func (v *Vector2) Dot(src d2interface.Vector2) *big.Float {
	c := v.Clone()
	c.X().Mul(c.X(), src.X())
	c.Y().Mul(c.Y(), src.Y())
...
// Cross Calculate the cross product of this Vector2 and the given Vector2.
func (v *Vector2) Cross(src d2interface.Vector2) *big.Float {
	c := v.Clone()
	c.X().Mul(c.X(), src.X())
	c.Y().Mul(c.Y(), src.Y())
...
// Lerp Linearly interpolate between this Vector and the given Vector2.
func (v *Vector2) Lerp(
	src d2interface.Vector2,
	t *big.Float,
) d2interface.Vector2 {
...
func (v *Vector2) Reflect(normal d2interface.Vector2) d2interface.Vector2 {
	clone := v.Clone()
	clone.Normalize()
...
// Mirror reflect this Vector2 across another.
func (v *Vector2) Mirror(axis d2interface.Vector2) d2interface.Vector2 {
	return v.Reflect(axis).Negate()
}

// Rotate this Vector by an angle amount.
func (v *Vector2) Rotate(angle *big.Float) d2interface.Vector2 {
