...
type KeyMap struct {
	...
	controls map[d2enum.GameEvent][]d2enum.Key
}
...
func NewKeyMap() *KeyMap {
	return &KeyMap{
		...
		controls: make(map[d2enum.GameEvent][]d2enum.Key),
	}
}

// SetBinding1 binds the first key for gameEvent
func (km *KeyMap) SetBinding1(gameEvent d2enum.GameEvent, key d2enum.Key) {
	if key == d2enum.KeyEscape {
		return
	}

	if km.controls[gameEvent] == nil {
		km.controls[gameEvent] = make([]d2enum.Key, 2)
	}

	currentKey := km.controls[gameEvent][0]
	delete(km.mapping, currentKey)
	km.mapping[key] = gameEvent

	if km.controls[gameEvent][1] == key {
		km.controls[gameEvent][1] = d2enum.Key(-1)
	}

	km.controls[gameEvent][0] = key
}

// SetBinding2 binds the second key for gameEvent
func (km *KeyMap) SetBinding2(gameEvent d2enum.GameEvent, key d2enum.Key) {
	if key == d2enum.KeyEscape {
		return
	}
	...
	currentKey := km.controls[gameEvent][1]
	delete(km.mapping, currentKey)
	km.mapping[key] = gameEvent

	if km.controls[gameEvent][0] == key {
		km.controls[gameEvent][0] = d2enum.Key(-1)
	}

	km.controls[gameEvent][1] = key
}
...
func (km *KeyMap) GetKeysForGameEvent(gameEvent d2enum.GameEvent) []d2enum.Key {
	return km.controls[gameEvent]
}

var defaultControls = map[d2enum.GameEvent][2]d2enum.Key{
