...
func (g *GameControls) OnMouseButtonRepeat(event d2interface.MouseEvent) bool {
	...
	isInActiveMenuRect := !g.isInActiveMenusRect(event.X(), event.Y())

	if !isInActiveMenuRect {
		return true
	}

	now := d2common.Now()
	shouldHandleLeft := now-lastLeftBtnActionTime >= mouseBtnActionsTreshhold
	shouldHandleRight := now-lastRightBtnActionTime >= mouseBtnActionsTreshhold

	if !(shouldHandleLeft || shouldHandleRight) {
		return true
	}

	button := event.Button()
	isLeftButton := button == d2interface.MouseButtonLeft
	isRightButton := button == d2interface.MouseButtonRight

	if isLeftButton && shouldHandleLeft && isInActiveMenuRect {
	...
	if isRightButton && shouldHandleRight && isInActiveMenuRect {
